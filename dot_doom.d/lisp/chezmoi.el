...
(defun chezmoi-apply-file (&optional file)
  (interactive)
  "applies the supplied file,
   if no file is given then the current buffer is used"
  (let ((file (or file (buffer-file-name))))
    (if (y-or-n-p (format "do you want to apply %s?" file))
        (progn (message (format "applying %s" file))
               (shell-command (format "chezmoi apply %s" file))))))

(defun chezmoi|write (&optional selected-file)
  "Run =chezmoi apply= on the target file associated with the buffer, overwriting the target with the source state."
  (interactive)
  (let ((selected-file (or selected-file (setq-local selected-file (buffer))))))
  ( if (y-or-n-p (format "do you want to apply %s?" (chezmoi|selected-file)))
      (progn
        (shell-command(concat "chezmoi apply " selected-file))
...
;;   (interactive)
;;   (let ((chezmoi|selected-file (or (chezmoi|selected-file (buffer-file-name)))))
;;     (replace-buffer-contents  nnfind-file-noselect chezmoi|selected-file))))
